...
router.get('/add', ensureAuthenticated, (req, res, next) => {
    try {
        ...
        error(`/add ERROR for ${req.body.notekey} ${req.body.title} ${req.body.body} ${e.stack}`);
        next(e); 
    }
});
...
router.post('/save', ensureAuthenticated, async (req, res, next) => {
    try {
        debug(`SAVE /notes/save user=${req.user ? req.user : "NOBODY"}`);
        var note;
        ...
        error(`/save ERROR for ${req.body.notekey} ${req.body.title} ${req.body.body} ${e.stack}`);
        next(e); 
    }
});
...
router.get('/view', async (req, res, next) => {
    ...
    throw new Error('This should make an unhandled Promise rejection');
});
...
router.get('/edit', ensureAuthenticated, async (req, res, next) => {
    try {
        ...
        error(`/edit ERROR for ${req.body.notekey} ${req.body.title} ${req.body.body} ${e.stack}`);
        next(e); 
    }
});
...
router.get('/destroy', ensureAuthenticated, async (req, res, next) => {
    try {
        ...
        error(`/destroy ERROR for ${req.query.key} ${e.stack}`);
        next(e); 
    }
});
